...
- Account information: Username (required), password (stored as a bcrypt hash), optional email address, and optional display name and profile image. If you sign in with Discord, Apple, or Google, we receive basic profile information from the provider (e.g., account identifier, username, email, avatar) and may store OAuth tokens required to facilitate sign‑in. If you register and/or use passkeys, we store passkey credential identifiers, public keys, counters, friendly names, and related metadata needed to verify sign‑ins; we do not store your private keys.
...
- Technical and security data: Session cookies; limited IP address data derived from request headers for login rate limiting and security; a normalized login identifier (e.g., username or email) used with IP address for short‑term abuse prevention; passkey registration and authentication challenges and related metadata needed to complete those flows; and basic request logs needed to operate the Service. Because we proxy the Service through Cloudflare (our CDN and security provider), Cloudflare also processes technical information necessary to deliver the Service and protect it, including your IP address, approximate location (e.g., country/region), User‑Agent, referrer, request identifiers (e.g., CF‑Ray), error and firewall events, and challenge/verification results used to distinguish bots from legitimate traffic.
...
- Authenticate users via username/password and OAuth providers (e.g., Discord, Apple, Google), and, where enabled, passkeys (WebAuthn), and maintain session security.
...
- Authentication: NextAuth handles username/password and OAuth sign‑in (e.g., Discord, Apple, Google). With the MongoDB adapter enabled, OAuth account records (including provider identifiers and tokens) may be persisted to support sign‑in. Passkey support uses WebAuthn‑based credentials (public keys and related metadata) associated with your account as described above. We do not post content to these providers on your behalf.
...
- Rate limiting: We derive an IP address from request headers and temporarily pair it with a normalized login identifier (e.g., username or email) to enforce sign‑in attempt limits. Records auto‑expire and are not persisted to long‑term storage.
