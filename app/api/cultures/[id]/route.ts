...
import CultureEntry from "@/models/CultureEntry";
...
export async function PATCH(request: Request, context: RouteContext) {
    ...
    try {
        ...
        const entry = await CultureEntry.findOne({ _id: ensureObjectId(id), userId: session.user.id });
        ...
        if ("name" in updates) {
            const name = sanitizeString(updates.name);
            if (name) {
                entry.name = name;
            }
        }

        if ("cultureType" in updates) {
            const allowedTypes = new Set(["roach", "isopod", "cricket", "mealworm", "superworm", "other"]);
            if (allowedTypes.has(updates.cultureType)) {
                entry.cultureType = updates.cultureType;
            }
        }
        ...
        if ("quantity" in updates) {
            entry.quantity = toNumber(updates.quantity);
        }

        if ("purchaseDate" in updates) {
            entry.purchaseDate = toDate(updates.purchaseDate);
        }

        if ("lastFed" in updates) {
            entry.lastFed = toDate(updates.lastFed);
        }

        if ("lastCleaned" in updates) {
            entry.lastCleaned = toDate(updates.lastCleaned);
        }
        ...
        console.error(error);
        return NextResponse.json({ error: "Unable to update culture entry." }, { status: 500 });
    }
}
...
export async function DELETE(_request: Request, context: RouteContext) {
    ...
    try {
        ...
        await connectMongoose();
        const entry = await CultureEntry.findOneAndDelete({
        ...
        console.error(error);
        return NextResponse.json({ error: "Unable to delete culture entry." }, { status: 500 });
    }
}
