...
function LoginForm() {
  ...
  const [providers, setProviders] = useState<Record<string, OAuthProviderConfig> | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/auth/csrf").then((res) => res.json()).catch(() => ({} as { csrfToken?: string })),
    ])
      .then(([providerData, csrfData]) => {
        if (!active) {
          return;
        }
        setProviders((providerData || {}) as Record<string, OAuthProviderConfig>);
        setCsrfToken(typeof csrfData?.csrfToken === "string" ? csrfData.csrfToken : null);
      })
      .catch(() => {
        if (!active) {
          return;
  ...
  const handleOAuthSignIn = (providerId: "apple" | "discord" | "google") => {
    if (typeof document === "undefined") {
      return;
    }

    const provider = providers?.[providerId];
    if (!provider?.signinUrl || !csrfToken) {
      setError("OAuth sign-in is temporarily unavailable. Please refresh and try again.");
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end">
          <Link
            ...
            className="text-xs text-[rgb(var(--primary))] hover:brightness-110 transition-all font-medium"
          >
            Forgot Password?
          </Link>
        </div>
      ...
      {/* Apple OAuth */}
      {
        !!providers?.apple && (
          ...
          >
            ...
            {oauthLoading === "apple" ? "Continuing with Apple..." : "Continue with Apple"}
          </Button>
        )
      }
      ...
      {/* Discord OAuth */}
      {
        !!providers?.discord && (
          ...
          >
            ...
            {oauthLoading === "discord" ? "Continuing with Discord..." : "Continue with Discord"}
          </Button>
        )
      }
