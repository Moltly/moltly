const LOCAL_STORAGE_KEY = "moltly:guest-health";
...
export function readLocalHealthEntries(): UnknownRecord[] {
...
export function writeLocalHealthEntries(entries: UnknownRecord[]): void {
  const storage = getLocalStorage();
  if (!storage) return;
  try {
